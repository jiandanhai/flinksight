// src/main/java/com/flinksight/backend/service/cluster/probe/YarnClusterHealthProbe.java
...
public class YarnClusterHealthProbeImpl implements ClusterHealthProbe {
  ...
  @Override public boolean supports(String type) {
    return "YARN".equalsIgnoreCase(type);
  }
  ...
  public ClusterHealthDTO check(ClusterDTO c) throws Exception {
    ...
    JsonNode info = getJson(base + "/ws/v1/cluster/info").path("clusterInfo");

    Map<String, Object> samples = new HashMap<>();
    samples.put("haState", info.path("haState").asText(null));
    samples.put("state",   info.path("state").asText(null));
    samples.put("id",      info.path("id").asLong(0));
    samples.put("rmStateStoreName", info.path("rmStateStoreName").asText(null));

    ClusterHealthDTO.Status status =
        "STARTED".equalsIgnoreCase(info.path("state").asText()) ? ClusterHealthDTO.Status.UP : ClusterHealthDTO.Status.DEGRADED;
