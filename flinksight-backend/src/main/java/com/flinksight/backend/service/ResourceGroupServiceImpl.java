...
public class ResourceGroupServiceImpl implements ResourceGroupService {
    ...
    @Override
    public PageResult<ResourceGroupDTO> list(int page, int size) {
        PageRequest pr = PageHelpers.pageRequest(page, size, null, ResourceGroup.class); // 统一 1→0
        Page<ResourceGroup> result = repository.findByTenantIdAndIsDeleted(SecurityUtil.getCurrentTenantId(),0, pr);
        return PageHelpers.toPageResult(result, resourceGroupStructMapper::toDTO, true); // 返回
    }
